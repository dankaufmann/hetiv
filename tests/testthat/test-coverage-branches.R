test_that("simulatedata covers alternative shock distributions and errors", {
  ...
  expect_error(
    simulatedata(
      Phi, 1, PsiE, PsiR, 10, 1, N, R, E, 5, P,
      c(0.8, 0.3), 1
    ),
    "GARCH parameters"
  )
  expect_error(
    simulatedata(
      Phi, 1, cbind(PsiE, PsiE), PsiR, 10, 1, N, R, 2, 5, P,
      c(0.1, 0.3), 1
    ),
    "GARCH shocks"
  )
  expect_error(
    simulatedata(
      array(0, dim = c(N, N, 20)), 1, PsiE, PsiR, 10, 1,
      N, R, E, 5, 20, 0, 1
    ),
    "P must be smaller"
  )
...
test_that("exported functions surface validation errors clearly", {
  ...
  expect_error(
    hetiv(y = "bad", O = y, Ind = Ind, P = 1, H = 2),
    "y must be numeric"
  )
  expect_error(
    hetiv(y = y, O = y[-1, ], Ind = Ind, P = 1, H = 2),
    "O must have"
  )
  expect_error(
    hetiv(
      y = y, O = y, X = y[-1, 1, drop = FALSE], Ind = Ind,
      P = 1, H = 2
    ),
    "X must have"
  )
  expect_error(
    hetiv(y = y, O = y, Ind = Ind, P = 1.5, H = 2),
    "P must be an integer"
  )
  expect_error(
    hetiv(y = y, O = y, Ind = Ind, P = 1, H = 2, E = 3),
    "E cannot exceed"
  )
  expect_error(
    hetiv(y = y[1:3, ], O = y[1:3, ], Ind = Ind[1:3], P = 1, H = 2),
  ...
  expect_error(
    proxyiv(
      y = y, O = y, Z = matrix(rnorm(nrow(y)), ncol = 1),
      Ind = Ind, P = 1, H = 2, E = 2
    ),
  ...
  expect_error(
    computeirf(
      Psi = matrix(1, 2, 1), Phi = matrix(0, 2, 2),
      H = 2, cum = FALSE
    ),
    "Phi must be"
  )
  ...
  expect_error(
    filllinear(data.frame(x = "bad", stringsAsFactors = FALSE), gap = 1),
    "All columns"
  )
  expect_error(
    filllinear(data.frame(x = c(1, NA, 3)), gap = 1.5),
    "gap must be"
  )
...
test_that("estimators cover optional controls, cumulation, interactions, and HAC covariance", {
  ...
  het <- hetiv(
    y = y, O = y, X = X, Ind = Ind, P = 0, H = 3,
    interact = TRUE, cum = c(TRUE, FALSE), cov_type = "NW"
  )
  prox <- proxyiv(
    y = y, O = y, X = X, Z = Z, Ind = Ind, P = 0, H = 3,
...
test_that("kfpredict covers unscaled and weak-heteroskedasticity branches", {
  ...
  raw <- kfpredict(
    Sig = Sig, SigR = diag(2), Psi = Psi, et = et,
    scale = FALSE, tol = 0
  )
  ...
  expect_warning(
    scaled <- kfpredict(
      Sig = diag(c(0.5, 0.5)), SigR = diag(2),
...
test_that("gweakivtest covers HAC, relative criterion, targeted beta, and helpers", {
  mat_pow <- getFromNamespace(".mat_pow", "hetiv")
  nearest_spd <- getFromNamespace(".nearestSPD", "hetiv")
  my_qr <- getFromNamespace(".myQR", "hetiv")
  opt_stiefel_gbb <- getFromNamespace(".OptStiefelGBB", "hetiv")

  expect_error(mat_pow(matrix(1, 2, 3), 0.5), "square")
  expect_error(mat_pow(diag(c(1, -1)), 0.5), "positive semidefinite")
  expect_error(
    mat_pow(matrix(c(1, 0, 0, 0), 2, 2), -0.5),
    "positive definite"
  )
  ...
  expect_error(
    gweakivtest(y, cbind(Y, Y), X, Z[, 1, drop = FALSE]),
    "Not identified"
  )
